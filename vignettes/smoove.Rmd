---
title: "smoove"
author: "Eliezer Gurarie and Faridedin Cheraghi"
date: "`r Sys.Date()`"
...
The `smoove` package is a collection of functions to work with continuous time correlated velocity movement (CVM) models as described in Gurarie et al 2017, *Movement Ecology*.   There are functions that simulate, diagnose and estimate these movement models, as well as functions for facilitating a change point analysis. 

# velocity movement models
...
```{r FirstPlot, echo=-1, fig.height=3, cache=TRUE}
...
```{r ucvm.exact, echo=-1, fig.height=4, cache=TRUE}
...
```{r plotRACVM, cache=TRUE, echo=-1, fig.height=4, bty="l"}
...
# Empirical velocity auto-correlation function
...
```{r EVAFplots, echo = -1, cache=TRUE, fig.width=10}
...
The velocity autocovariance functions is a very useful visual summary of a movement track because it refeals . Furthermore These curves can be used to estimate the parameters of these processes. 
...
```{r VAF1, fig.height=4, echo=-1, cache=TRUE}
...
```{r acvm, cache=TRUE, echo=FALSE, fig.height = 3}
...
```{r racvm, cache=TRUE, echo=-1, fig.height = 3}
...
In the paper, we analyze a phenomenal data set of a single lesser kestrel (*Falco naumanni*) flight in southern Spain from Hern{\'a}ndez-Pliego et al (2015a, 2015b). One kestrel's flight is in the package:
...
```{r K1, echo=-1, fig.height=4, cache=TRUE}
...
```{r K1.sim, echo=-1, fig.height=4, cache=TRUE}
par(mar = c(0,2,0,0), oma = c(4,4,2,2), bty="l")
...
```{r TwoPhaseCVM, fig.height = 4, echo=-1, out.width = "6in", cache=TRUE, fig.align="center"}
par(bty="l")
...
```{r fig.height = 4, echo=-1, cache=FALSE}
par(bty="l")
...
```{r plotMulticvm, echo =-1, fig.height = 4, fig.width = 8, cache=TRUE, fig.align="center"}
...
```{r loadsimSweep, echo=-1, fig.height=3}
...
### Step II - Obtain Candidate Change Points 
...
### Step IV - estimating the full model 
...
```{r SimPhasePlot, message=FALSE, echo = -1}
...
```{r subsetKestrel, echo=-1, fig.height=4}
...
```{r plotKestrelSweep, echo=-1, fig.height=3}
...
```{r kestrelMegaPlot, echo=-1, eval=TRUE, warning = FALSE}
...
- Hern{\'a}ndez-Pliego, J., C.~Rodr{\'i}guez, J.~Bustamante (2015) 'Data from: Why do kestrels soar?', Movebank Data Repository. \url{https://www.datarepository.movebank.org/handle/10255/move.486}
- Hern{\'a}ndez-Pliego, J., C.~Rodr{\'i}guez, J.~Bustamante (2015) 'Why do kestrels soar?', PLoS ONE 10(12).
