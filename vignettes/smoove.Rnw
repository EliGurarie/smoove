...
@
\noindent The {\tt plot.track} function is a simple convenience function for plotting tracks.  According to theory, the mean speed of this track should just be $\nu$ = \Sexpr{nu}:

<<>>=
mean(Mod(ucvm1$V))
@

\noindent and the root mean square speed should be $2 \nu / \sqrt{\pi}$ = \Sexpr{nu * 2 /sqrt(pi)}:
...
@
\noindent  The \texttt{scan.track} function is another function that is convenient for seeing a process in time (right plots).  Note the irregularity of the sampling. 
<<>>=
...
@
\noindent The confidence intervals are more wide this time (due to a much smaller data set: \Sexpr{length(ucvm2$Z)} versus \Sexpr{length(ucvm1$Z)} data points), but the estimates should be fairly accurate. 
...
@
\noindent Visually, the simulation looks rather similar with respect to radii and frequence of rotations, randomness and advective tendency to the data snippet.  
...
\noindent The true change point occured at time: 
<<>>=
max(ucvm1$T)
@
...
To find multiple change points where the number of changes is unknown \emph{a priori}, we sweep change point analysis windows across the entire time series to obtain candidate change points.   Simulate an all-UCVM trajectory with 3 behavioral phases:

<<simSweepGenerate, echo=-1, out.width = "4in", fig.height = 4, fig.width = 4, cache=FALSE, fig.align="center">>=
par(bty="l"); palette(rainbow(4)); set.seed(9) #set.seed(101)
n <- 200
...
multicvm <- data.frame(Z = Z.raw, T = T.raw)[sample(1:length(Z.raw), 600),] %>% arrange(T)

plot.track(multicvm$Z, col = cut(multicvm$T, c(0,cumsum(tmaxes))))
legend("bottom", pch = 19, col=1:3, legend = c("I","II","III"), 
       title = "true phase", bty="n", ncol=3)
@

<<eval=FALSE, echo = FALSE>>=
#ucvm1 <- simulateUCVM(T=cumsum(rexp(n)), nu=1, tau=3, method="exact")
#ucvm2 <- simulateUCVM(T=cumsum(rexp(n)), nu=5, tau=10, v0 = ucvm1$V[n], method="exact")
#ucvm3 <- simulateUCVM(T=cumsum(rexp(n)), nu=5, tau=3, v0 = ucvm2$V[n], method="exact")

#T <- c(ucvm1$T, ucvm1$T[n] + ucvm2$T, ucvm1$T[n] + ucvm2$T[n] + ucvm3$T)
Z <- c(ucvm1$Z, ucvm1$Z[n] + ucvm2$Z, ucvm1$Z[n] + ucvm2$Z[n] + ucvm3$Z)
plot.track(Z)

@

\noindent The actual change points occur at times \Sexpr{round(max(ucvm1$T),2)} and  \Sexpr{round(max(ucvm1$T + ucvm2$T),2)}.
...
Z <- multicvm$Z
T <- multicvm$T
simSweep <- sweepRACVM(Z=Z, T=T, windowsize = 100, windowstep = 10, model = "ACVM", progress=FALSE)
@
<<eval=FALSE, include=FALSE>>=
save(simSweep, file = "simSweep.robj")
@

\noindent Toggling the \texttt{progress} to TRUE will output a series of progress bars in the R console.  

<<echo=-1, fig.height=3>>=
par(bty="l")
plotWindowSweep(simSweep)
@
\noindent In the image above, each color represents the relative log-likelihood profile for a single window.  There are two distinct peaks, the remainder of the algorithm boils these down to ``significant'' change points. 

\subsubsection{Step II - Obtain Candidate Change Points} 

We obtain candidate change points by taking all of the most likely change points (MLCP) in each of the windows, listing all of the MLCP's.  The function that does this is \texttt{findCandidateChangePoints}.  All the candidates:
<<>>=
CP.all <- findCandidateChangePoints(windowsweep = simSweep, clusterwidth = 0)
CP.all
@
\noindent The message tells us that The warning lets us know that some of the candidate change points are rather close to each other, and it might be a good idea to cluster them, i.e.~assign multiple points within a given time interval to a single cluster. If we take a rather generous cluster width below: 
<<>>=
CP.clustered <- findCandidateChangePoints(windowsweep = simSweep, clusterwidth = 5)
CP.clustered
@
\noindent We now have a more limited set of change points to assess.  
...
getCPtable(CPs = CP.clustered, Z = Z, T = T, modelset = c("UCVM", "ACVM"), tidy = "BIC", iterate = TRUE)
@ 

\noindent We now have a final selected set of change points and, perhaps more usefully, a summary of which parameters precisely changed.  Note that thanks to the magic of \texttt{magrittr} piping, the process of finding and selecting change points can be reduced to a single line of code, which is convenient for comparing the robustness of the different settings.  Replicating the above analysis for several cluster widths yields the same basic result:

<<exampleCPtables, cache=TRUE>>=
simSweep %>% findCandidateChangePoints(clusterwidth = 2) %>% 
  getCPtable(Z = Z, T = T, modelset = c("UCVM", "ACVM"), tidy = "BIC")
simSweep %>% findCandidateChangePoints(clusterwidth = 4) %>% 
  getCPtable(Z = Z, T = T, modelset = c("UCVM", "ACVM"), tidy = "strict")
simSweep %>% findCandidateChangePoints(clusterwidth = 10) %>% 
  getCPtable(Z = Z, T = T, modelset = c("UCVM", "ACVM"), tidy = "BIC")
@

\noindent Compared to the true values of \Sexpr{tmaxes[1]} and  \Sexpr{sum(tmaxes[1:2])}, these are excellent estimates. We can also expand our "model set" to include all four CVM model:

<<simSweepAllModels, cache=TRUE>>=
simSweep %>% findCandidateChangePoints(clusterwidth = 10, verbose = FALSE) %>% getCPtable(Z = Z, T = T, modelset ="all", tidy = "strict")
@

\noindent But the main model selected in most significant partition is still the UCVM.  Note that because this analyses is selecting from multiple models (of which, in this case, only one is the correct one), it is more likely to give a "spurious" (i.e. overdetermined) analysis. 

\subsubsection{Step IV - estimating the full model}  

The final, straightforward, step is simply to estimate the parameters for the model within each of the phases defined by the selected change points, and thereby obtain a fully parameterized model of the movement.  This is done with the \texttt{getPhases} function, which takes a change point table (i.e.~the output of \texttt{getCPtable}]) and returns a named list of phases and the estimated parameters:

<<>>=
simCP.table <- simSweep %>% findCandidateChangePoints(clusterwidth = 5, verbose = FALSE) %>% 
...
\noindent The estimates are fairly accurate with respect to the true $\tau$ and $\eta$ values (1-5-1 and 2-5-5) respectively. 
...
\noindent Note that the selected cluster width (2) was the minimum one that did not produce a warning for clusters that were ``too close''.  We now iteratively test all of the change points for parameter values and model changes.  Specifying \texttt{modelset = `all'} is a shortcut for  \texttt{modelset = c(`UCVM', `ACVM', `RCVM', `RACVM')}: 
...
\item[-] Gurarie, E., C.~Fleming, K.~Laidre, W.~Fagan, O.~Ovaskainen (in review) \emph{Methods in Ecology and Evolution}. 
