...
\noindent The actual change points occur at times \Sexpr{durations[1]} and  \Sexpr{durations[1] + durations[2]}.
...
\noindent The warning lets us know that some of the candidate change points are rather too close (in time) to each other, and it might be a good idea to cluster them, i.e.~assign multiple points within a given time interval to a single cluster.  NB: the output object ``carries'' with it the location and time data as ``attributes'' - the \texttt{as.vector} command simplies tidies the output. 
...
\noindent We now have (probably) a slightly smaller set of change points to assess.  
...
\noindent Which (using AIC as the criterion) suggests that the second phase might, in fact, be rotational.  Note that because this analysis is selecting from multiple models (of which, in this case, only one is the correct one), it is more likely to give a "spurious" (i.e. overdetermined) analysis. 

\subsubsection{Step IV - estimating the full model} 

The final step is to estimate the parameters for the model within each of the phases defined by the selected change points, and thereby obtain a fully parameterized model of the movement.  This is done with the \texttt{estimatePhases} function, which takes a change point table (i.e.~the output of \texttt{getCPtable}]) and returns a named list of phases and the estimated parameters:
...
The \texttt{simPhaselist} object is a list that contains more information about each phase, notably, confidence intervals:

<<>>=
simPhaselist[[1]]
@

\noindent  To actually obtain (and not merely print) the summary table, you need to run:
<<>>=
...
\noindent A subtle nuance: Leaving the \texttt{xaxt= "n"} out provided a nicely formatted time axis.  Because R is R, if you try doing this post-facto (e.g., via \texttt{axis(1)}), it won't quite work. 
