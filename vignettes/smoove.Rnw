...
This is a good estimate of the true change point, which occurred at time \Sexpr{max(ucvm1$T)}.
...
\noindent The actual change points occur at times \Sexpr{tmaxes[1]} and  \Sexpr{tmaxes[1] + tmaxes[2]}.
...
<<simSweep.parallel, cache=TRUE, warning = FALSE, eval=FALSE>>=
...
<<>>=
CP.clustered <- findCandidateChangePoints(windowsweep = simSweep, clusterwidth = 2)
CP.clustered
@
...
\noindent Compared to the true values (\Sexpr{tmaxes[1]} and  \Sexpr{sum(tmaxes[1:2])}), these are excellent estimates of the change points themselves.  However, 

We can also expand our "model set" to include all four CVM model:

<<simSweepAllModels, cache=TRUE>>=
simSweep %>% findCandidateChangePoints(clusterwidth = 10, verbose = FALSE) %>% getCPtable(Z = Z, T = T, modelset ="all")
@
...
Note that the analysis did not correctly identify 
...
simCP.table <- simSweep %>% 
  findCandidateChangePoints(clusterwidth = 2, verbose = FALSE) %>% 
...
<<echo=-(1:2), message=FALSE>>=
  ...
  plotPhaseParameter("rms", simPhases,  ylab="distance / time", xlab="time", col=cols)
@
