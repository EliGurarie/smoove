...
#' @param {T.max,dt} maximum time and time interval of simulations.
#' @param T vector of time observations, needed if \code{T.max} and \code{dt} are NULL.  This vector can be irregular for an \code{exact} sampling.
#' @param method one of \code{direct}, referring to a direct numerical integration of the process which requires a fixed (and small) interval, or \code{exact} which samples from the likelihood of the complete process and can take any random vector or times \code{T}.  The "exact" sampling can be much slower for a large vector of observations.  See \code{\link{simualteUCVM.exact}} and the package vignette for more details. 
...
#' 
#' ## Also plotting the velocity and using the plot.track function.
#' par(mfrow=c(1,2))
#' plot(ucvm$V[1:1000], asp=1, type="l", main = "Velocity")
#' plot.track(ucvm$Z[1:1000], asp=1, pch=19, cex=0.5, col=rgb(0,0,0,.1), main = "Location")
#' 
...
#'     plot(T, Re(V), col=2, type="o", main="Velocity (decomposed)", ylim=range(c(Re(V), Im(V))),  pch=19, cex=0.5)
#'     lines(T, Im(V), col=3, type="o", pch=19, cex=0.5)
#'     plot(T, Re(Z), col=2, type="o", main="Position (decomposed)", ylim=range(XY), pch=19, cex=0.5)
