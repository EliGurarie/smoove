...
#' @param XY XY two-column matrix or specify Z
#' @param Z complex location vector or specify XY
...
#' @param ... further argument to \code{testCP}
...
#' @export
getCPtable <- function(CPs, XY = NULL, Z = NULL, T, modelset, tidy = "strict", iterate = TRUE, spline = FALSE, criterion = 'BIC', ...)
{
  ...
  while(!stop){
    ...
    for(i in 1:length(CPs)){
      CPanalysis <- testCP(Z, T, CPs[i], starts[i], ends[i], modelset = modelset, spline, criterion, ...)
    ...
    if(!is.null(tidy)){
      #RCMD check complains about NON standard Evaluation
