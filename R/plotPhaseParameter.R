...
#' @param log a character string which contains "y" if the y axis is to be logarithmic and "xy" or "yx" if both axes are to be logarithmic.
...
#' @export
#' 
plotPhaseParameter <- function(variable, phaselist, 
  ...
  if(any(low.plot == -Inf)){
    which.inf <- which(low.plot == -Inf)
    low.plot[low.plot == -Inf] <- 10*min(low.plot[low.plot != -Inf])
    warning(cat("The lower confidence limit for phase", as.roman(which.inf), 
                "is negative infinity and will be plotted as very negative. Consider setting y limits manually.\n"))
  }
  
  if(any(low.plot == 0) & log == "y"){
    which.zero <- which(low.plot == 0)
    low.plot[low.plot == 0] <- .0001
    warning(cat("The lower confidence limit for phase", as.roman(which.zero), 
                "is 0, which will be plotted on the log-scale as something very negative. Consider setting y limits manually.\n"))
  }
  
  if(any(high.plot == Inf)){
    which.inf <- which(high.plot == Inf)
    high.plot[high.plot == Inf] <- 10*max(high.plot[high.plot != Inf])
    warning(cat("The upper confidence limit for phase", as.roman(which.inf), "is infinity and will be plotted as very positive. Consider setting y limits manually.\n"))
  }
